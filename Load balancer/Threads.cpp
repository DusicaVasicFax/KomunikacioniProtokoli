...
#pragma endregion
			...
			*/
			...
			data->port = (char*)"21000";
			data->value = node->value;
			
			DWORD workerRole1Id;
			CreateThread(NULL, 0, &workerRole1, &data, 0, &workerRole1Id);
		}
		Sleep(1000);
	}
}
...
DWORD WINAPI workerRole1(LPVOID param)
{
	...
	serverAddress.sin_port = htons(atoi(parameters->port));
	...
	{
		printf("Message received from client: %s.\n", recvbuf);
		char* messageToSend = (char*)"test response";
	}
	else if (iResult == 0)
	{
		// connection was closed gracefully
